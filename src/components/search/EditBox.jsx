...
export default class EditBox extends React.Component {
  ...
  _modifyEntry(entryid, imdbid){
    ...
    fetch(`//${process.env.API_HOST}/collection/${this.props.id}/override/${entryid}/to/${imdbid}`, {
      method: 'put'
    }).then(response => {
  ...
  componentDidMount () {
    fetch(`//${process.env.API_HOST}/process/search?input=${this.props.movie.guess}&index=tmdb&type=movie&mode=match`)
    .then((response) => {
      if (response.status !== 200) throw new Error(`Server status: ${response.status}`)
      return response.json()
    })
